...
1. Загружаются данные в AppSate из ApiService
...
поля:\
— `catalog: IProductItem[]` — массив каточек товара\
— `basket: IProductItem[] = []` — каталог с купленными товарамие\
— `order: IOrder` — данные для отправки на сервер\
— `formErrors: FormErrors` — данные ошибок заполнения форм\

методы:\
— `setСatalog(item: IProductItem[]): void` — Установить каталог с товарами\
— `getCatalog(): IProductItem[]: void` — Получить каталог с товарами\
— `getProduct(id: string): IProductItem` — Получить карточку с товаром\
...
— `getBasketList(): IProductItem[]` — Вывод товаров находящихся в корзине\
...
Наследует класс Component<IProductItem>\
поля:\
